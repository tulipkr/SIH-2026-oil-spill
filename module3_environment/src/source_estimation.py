...
from .backward_advection import backward_trajectory
...
def _coverage(ds, lat, lon, start, end, margin, label):
    ...
    if requested[0] < min_lat or requested[1] > max_lat or requested[2] < min_lon or requested[3] > max_lon:
        raise ValueError(f"{label} spatial coverage insufficient: requested={requested}, available={(min_lat,max_lat,min_lon,max_lon)}")
    if "time" not in ds.coords:
        raise KeyError(f"{label} dataset has no time coordinate")
    times = np.asarray(ds.time.values)
    if len(times) == 0:
        raise ValueError(f"{label} dataset has no time values")
    t0 = np.datetime64(start.replace(tzinfo=None)); t1 = np.datetime64(end.replace(tzinfo=None))
    if t0 < times.min() or t1 > times.max():
        raise ValueError(f"{label} temporal coverage insufficient: requested={t0}..{t1}, available={times.min()}..{times.max()}")
    LOG.info("%s coverage: actual bbox=%s time=%s..%s; requested bbox=%s time=%s..%s", label, _bbox(ds), times.min(), times.max(), requested, t0, t1)
...
def run_pipeline(summary_path, geometry_path, era5_path=None, glorys_path=None, config_path=None, output_path=None):
    ...
    if era5_path is None:
        ...
        fetch_era5(output_path=era5_path, bbox=bbox, start_utc=start, end_utc=acquisition, cdsapi_config=config)
    if glorys_path is None:
    ...
    try:
        ...
        def velocity_at(when, lat, lon):
            ...
            values = []
            for name, da in (("ERA5 u10", u10_da), ("ERA5 v10", v10_da), ("GLORYS uo", uo_da), ("GLORYS vo", vo_da)):
                ...
                if used_nearest:
                    ...
                    if key not in nearest_logged:
                        LOG.warning("nearest-valid fallback used for %s at %s lat=%.5f lon=%.5f", name, when, lat, lon)
                        nearest_logged.add(key)
                values.append(float(value))

            u10, v10, uo, vo = values
            u = uo + float(config["wind_drift_factor"]) * u10
            v = vo + float(config["wind_drift_factor"]) * v10
